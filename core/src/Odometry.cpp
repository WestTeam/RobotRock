...
namespace
{
    HUMANAFTERALL_LOGGING_CATEGORY( LOG, "WestBot.RobotRock.Odometry" )
}

bool WestBot::RobotRock::operator==(RobotPos& lhs, RobotPos& rhs)
{
    /*
    if (((int16_t)lhs.x == (int16_t)rhs.x) &&
        ((int16_t)lhs.y == (int16_t)rhs.y) &&
        ((int16_t)(round(DEG((double)lhs.theta)*100.0)) == (int16_t)(round(DEG((double)rhs.theta)*100.0))))
        return true;
    else
        return false;
        */
...
void Odometry::setPosition(RobotPos pos)
{
...
void Odometry::addError(RobotPos pos)
{
    QMutexLocker locker( & _lock );
